...
def main():
    ...
    try:
        ...
        save_to_notepad(f"Mobile3 device bluetooth name: {mobile3_name}\n")
        ...
        if "LOADED" not in sim_state:
            save_to_notepad(f"Mobile3 device doesn't have SIM card - test skipped\n")
            save_to_notepad(header="TEST SKIPPED", color="yellow")
            save_to_excel(test_name, "Skipped", "Mobile3 device doesn't have SIM card")
            return
        ...
        time.sleep(5)
        save_to_notepad(f"Incoming call initiated from Mobile3 to Mobile1\n")
        ...
        # Try to click End button first
        found = click_on_device_regex(HU, "End")
        if not found:
            # If End button not found, try Red Phone icon
            commands = [
            ...
            for i, cmd in enumerate(commands):
                ...
                if i == 2:  # After icon tap
                    assert x != 0 and y != 0, f"End Call icon has not been found on display.\n"
                    save_to_notepad(f"End Call icon has been found and pressed!\n")
                    found = False
            ...
            save_to_notepad(f"End button has been found and pressed on HU display.\n")
        
        # Wait for call to be terminated
        time.sleep(3)
        save_to_notepad(f"Waiting for call termination\n")
        
        # Check if call was terminated successfully
        if found:
            success_message = f"Call was terminated successfully - Contacts screen is visible"
            ...
            assert False, f"Call termination failed - Contacts screen not visible.\n"
        ...
        # Cleanup recordings based on test result
