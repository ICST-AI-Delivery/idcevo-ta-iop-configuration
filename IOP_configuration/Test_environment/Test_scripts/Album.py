...
def main():
    ...
    try:
        ...
        # Extract the name of the current album playing from Mobile Device 1
        album_name = phone.album_name_command()
        save_to_notepad(f"Current album name from Mobile Device 1: {album_name}\n")
        
        # Check if current album name is displayed on HU
        found = find_word_on_device_via_regex(HU, album_name)        
        if found:
            success_message = f"Album name display functionality works correctly - test Passed."
            ...
            assert False, f"Album name '{album_name}' not found on HU display - album name display functionality failed - test Failed"
        ...
        found_bt = click_on_device_regex(HU, "Bluetooth")
        assert found_bt, f"Bluetooth button not found on HU display\n"
