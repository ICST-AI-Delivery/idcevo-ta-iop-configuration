...
test_name = "FFW_FRW_on_Mobile_Device"
...
def main():
    ...
    try:
        ...
        # Press fast forward on Mobile Device 1
        phone.fast_forward_command()
        save_to_notepad(f"Pressed fast forward on Mobile Device 1\n")
        time.sleep(3)
        
        # Press fast rewind on Mobile Device 1
        phone.fast_rewind_command()
        save_to_notepad(f"Pressed fast rewind on Mobile Device 1\n")
        time.sleep(3)
        
        # Check test result
        if rc == 0:
            success_message = f"Audio playback is initiated on HU, HU winds forwards, and HU rewinds - test Passed."
            ...
            assert False, f"Wind forward and rewind functionality is not working properly - test Failed"
