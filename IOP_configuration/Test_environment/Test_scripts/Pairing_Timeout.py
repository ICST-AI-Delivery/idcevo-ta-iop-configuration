...
def main():
    ...
    try:
        ...
        save_to_notepad(f"Waiting for Pairing timeout 30s...\n")
        time.sleep(30)

        # Check if "Try" word can be found on HU display
        found = find_word_on_device_via_regex(HU, "Try")
        if found:
            success_message = f"Pairing timeout was reached in maximum 30s."
            ...
            assert False, f"Pairing timeout was not reached in maximum 30s.\n"
