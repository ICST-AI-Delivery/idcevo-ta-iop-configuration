...
test_name = "Linkloss_during_phonebook_download"
...
def main():
    ...
    try:
        ...
        time.sleep(15)  # Wait for reconnection

        # Click Not now button from HU display one time (may not appear everytime)
        found = click_on_device(HU, "Not now")
        time.sleep(1)

        # Check if Contacts word can be found on HU display with regex after Bluetooth reconnection
        found = find_word_on_device_via_regex(HU, "Contacts")
        if found == True:
            success_message = f"{mobile_name} Phonebook is resuming download after Linkloss."
            ...
            assert False, f"{mobile_name} Phonebook download failed after Linkloss.\n"
