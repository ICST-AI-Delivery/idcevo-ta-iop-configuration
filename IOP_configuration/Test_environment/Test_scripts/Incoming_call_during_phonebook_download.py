...
test_name = "Incoming_call_during_phonebook_download"
...
def main():
    ...
    try:
        ...
        rc = phone2.dial_command(phone_number_mobile1)
        ...
        # Check if Loading word can be found on HU display with regex during incoming call
        found = find_word_on_device_via_regex(HU, "Contacts")
        if found == True:
            success_message = f"{mobile_name} Phonebook is downloading to HU during incoming call."
            ...
            assert False, f"{mobile_name} Phonebook download failed during incoming call.\n"
