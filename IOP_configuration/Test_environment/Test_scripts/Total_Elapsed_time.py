...
def main():
    ...
    try:
        ...
        save_to_notepad(f"Mobile device object created\n")
        
        # Start audio playback on Mobile Device 1
        rc = phone.play_audio_command()
        save_to_notepad(f"Started audio playback on Mobile device\n")
        time.sleep(3)
        ...
        save_to_notepad(f"Clicked Source button: {found}\n")
        time.sleep(2)
        ...
        save_to_notepad(f"Result: {stdout}\n") 
        assert rc == 0, f"Command {command} failed: {rc}\n"
        save_to_notepad(f"Clicked Bluetooth option at coordinates ({x}, {y})\n")
        time.sleep(3)
        
        
        # Check if elapsed time is displayed in the status bar on the lower right of the display
        elapsed_time_found = find_word_on_device_via_regex(HU, "[0:9]:")
        ...
        if elapsed_time_found:
            ...
            assert False, f"Playing time display functionality failed - test Failed"
        ...
        rc = phone.pause_audio_command()
        save_to_notepad(f"Paused audio playback on Mobile device\n")
        time.sleep(2)
        
        # Click Bluetooth button with regex on HU display
        found = click_on_device_regex(HU, "Bluetooth")
        save_to_notepad(f"Clicked Bluetooth button: {found}\n")
        time.sleep(2)
        ...
        save_to_notepad(f"Result: {stdout}\n") 
        assert rc == 0, f"Command {command} failed: {rc}\n"
        save_to_notepad(f"Switched from mobile playback to radio at coordinates ({x}, {y})\n")
        time.sleep(2)
