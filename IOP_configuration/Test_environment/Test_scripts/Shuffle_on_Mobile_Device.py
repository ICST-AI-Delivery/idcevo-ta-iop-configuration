...
test_name = "Shuffle_on_Mobile_Device"
...
def main():
    ...
    try:
        ...
        # Activate shuffle mode on Mobile Device 1
        shuffle_active = phone.activate_shuffle_mode_command()
        ...
        # Check test result
        if shuffle_active == True:
            success_message = f"Shuffle mode functionality works correctly - tracks are played in random order - test Passed."
            ...
            assert False, f"Shuffle mode activation failed - test Failed"
        ...
        save_to_notepad(f"Deactivated shuffle mode on Mobile Device 1\n")
        time.sleep(2)
