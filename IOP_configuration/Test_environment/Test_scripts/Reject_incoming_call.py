"""
AI-Generated Test Script for: Reject_incoming_call
Generated on: 2026-03-04 16:28:46

Test Case Information:
- Precondition: Mobile Device and HU are connected.
Synchronisation is finished.
- Description: 1 Initiate an incoming call from MD3 to MD1
2 Reject incoming call on IDCEvo
- Expected Result: 2 IDCEvo and MD indicate the rejected call
"""
...
test_name = "Reject_incoming_call"
...
def main():
    ...
    try:
        ...
        # Initiate incoming call from MD3 to MD1
        ...
        # Wait 5 seconds for the incoming call to be established
        time.sleep(5)
        save_to_notepad(f"Waited 5 seconds for incoming call to be established\n")
        ...
        # Click Reject Button from HU display
        found = click_on_device_regex(HU, "Reject")
        time.sleep(1)
        assert found == True, f"Reject button has not been found on HU display.\n"
        save_to_notepad(f"Reject button has been found and pressed on HU display.\n")       
        
        # Check if call was rejected by looking for rejected call indication on HU display
        found = find_word_on_device_via_regex(HU, "Contacts")
        if found:
            # If both HU and Mobile devices indicate the rejected call then the test is Passed
            success_message = f"IDCEvo and {mobile_name} indicate the rejected call successfully."
            ...
            assert False, f"Call rejection indication not found on HU display.\n"      
