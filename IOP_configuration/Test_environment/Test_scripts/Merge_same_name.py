    ...
    try:
        ...
        for line in lines:
            if "display_name=" in line:
                # Modified regex to capture full names including spaces, stopping at comma or end of string
        ...
        # Check if any contact appears more than once
        contact_counts = Counter(contact_names)
        duplicate_contact = None
        for contact_name, count in contact_counts.items():
            if count > 1:
                duplicate_contact = contact_name
                save_to_notepad(f"Contact '{contact_name}' appears {count} times in mobile device contacts.\n")
                break
        
        assert duplicate_contact is not None, "No duplicate contacts found in mobile device contact list.\n"
        ...
        # Check if duplicate_contact is found in data2 list
        if duplicate_contact in data2_list:
            # Click on the full contact name with verification from HU display
            click_target = duplicate_contact
            save_to_notepad(f"Contact '{duplicate_contact}' found in data2 list. Will click on full contact name.\n")
        else:
            ...
            for line in lines:
                if line.strip() and 'data3=' in line:
                    ...
                    if data3_value and data3_value in duplicate_contact:
                        last_name = data3_value
                        save_to_notepad(f"Found last name of duplicate contact from data3: '{last_name}'\n")
                        break
            
            if last_name is None:
                assert False, f"No last name of duplicate contact found in data3 results.\n"
            
            click_target = last_name
            save_to_notepad(f"Contact '{duplicate_contact}' not found in data2 list. Will click on last name: '{last_name}'\n")

        # Scroll down on HU display 10 times using the specified coordinates
        for i in range(10):
            # Check how many times the duplicate contact appears on HU display
            contact_count_on_hu = count_word_occurrences_on_device(HU, click_target)
            time.sleep(1)
            if contact_count_on_hu == 0:
        ...
        save_to_notepad(f"Contact '{duplicate_contact}' found {contact_count_on_hu} times on HU display.\n")
        
        # If contact is found only once on HU, then contacts with same name were merged successfully
        if contact_count_on_hu == 1:
            success_message = f"{mobile_name} Contacts with the same name were merged successfully on HU."
            ...
            assert False, f"{mobile_name} Contacts with the same name were not merged on HU. Found {contact_count_on_hu} occurrences instead of 1.\n"
