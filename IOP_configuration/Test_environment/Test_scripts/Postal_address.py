...
def main():
    ...
    try:
        ...
        for line in lines:
            if "display_name" in line:
                ...
                if match:
                    ...
                    save_to_notepad(f"First contact with postal address: {contact_name}\n")
                    break
        ...
        # Check if contact_name is found in data2 list
        if contact_name in data2_list:
            # Click on the full contact name with verification from HU display
            click_target = contact_name
            save_to_notepad(f"Contact '{contact_name}' found in data2 list. Will click on full contact name.\n")
        else:
            ...
            for line in lines:
                if line.strip() and 'data3=' in line:
                    ...
                    if data3_value and data3_value in contact_name:
                        ...
                        save_to_notepad(f"Found last name with postal address from data3: '{last_name}'\n")
                        break
            
            if last_name is None:
                assert False, f"No last name with postal address found in data3 results.\n"
            
            click_target = last_name
            save_to_notepad(f"Contact '{contact_name}' not found in data2 list. Will click on last name: '{last_name}'\n")
        ...
        assert found == True, f"{click_target} button has not been found on HU display.\n"
        save_to_notepad(f"{click_target} button has been found and pressed on HU display.\n")

        # Check if "Private" word can be found on HU display with regex
        found = find_word_on_device_via_regex(HU, "Private")
        if found == True:
            success_message = f"{mobile_name} Contact with postal address is displayed successfully on HU."
            ...
            assert False, f"Mobile device Contact with postal address is not displayed correctly on HU.\n"
