...
def main():
    ...
    try:
        ...
        adb_command = f"adb -s {HU}" + r' shell getprop ro.build.fingerprint'
        ...
        for cmd in commands:
            ...
            if cmd == commands[1]:
                # If adb command returns the Firmware of the HU the test is passed, else test is failed
                assert stdout and len(stdout.strip()) > 0, "Firmware fingerprint not found or empty!"
                success_message = f"Firmware successfully retrieved: {stdout}"
