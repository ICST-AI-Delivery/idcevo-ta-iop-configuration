...
def main():
    ...
    try:
        ...
        stdout = phone.get_contacts_name_and_photo_id_command()
        assert rc == 0, f"Query contacts command failed: {rc}\n"
        
        # Extract the first display_name for which photo_id is not NULL
        contact_with_photo = None
        contact_id = None
        
        lines = stdout.strip().split('\n')
        for line in lines:
            if line.strip() and '=' in line:
                # Parse the line format: Row: 0 _id=X, display_name=Y, photo_id=Z
                # or similar variations
                fields = line.split(',')
                display_name = None
                photo_id = None
                contact_id_temp = None
                
                for field in fields:
                    field = field.strip()
                    if field.startswith('_id=') or ' _id=' in field:
                        contact_id_temp = field.split('=', 1)[1].strip()
                ...
                for field in fields:
                    field = field.strip()  
                    if 'photo_id=' in field:
                        photo_id_value = field.split('=', 1)[1].strip()
                        # Check if photo_id is not NULL, not empty, and greater than 0
                        if photo_id_value and photo_id_value.lower() != 'null' and photo_id_value != '' and int(photo_id_value) > 0:
                            photo_id = photo_id_value

                # If we found a contact with both display_name and non-NULL photo_id, use it
                if display_name and photo_id and contact_id_temp:
                    contact_with_photo = display_name
                    contact_id = contact_id_temp
                    save_to_notepad(f"Found first contact with photo: '{contact_with_photo}' (ID: {contact_id}, photo_id: {photo_id})\n")
                    break
        
        if contact_with_photo is None:
            assert False, f"No contact found with non-NULL photo_id in mobile device contacts.\n"
        ...
        # Check if contact_with_photo is found in data2 list
        if contact_with_photo in data2_list:
            # Click on the full contact name with verification from HU display
            click_target = contact_with_photo
            save_to_notepad(f"Contact '{contact_with_photo}' found in data2 list. Will click on full contact name.\n")
        else:
            ...
            for line in lines:
                if line.strip() and 'data3=' in line:
                    ...
                    if data3_value and data3_value in contact_with_photo:
                        last_name = data3_value
                        save_to_notepad(f"Found last name with contact photo from data3: '{last_name}'\n")
                        break
            
            if last_name is None:
                assert False, f"No last name with contact photo found in data3 results.\n"
            
            click_target = last_name
            save_to_notepad(f"Contact '{contact_with_photo}' not found in data2 list. Will click on last name: '{last_name}'\n")

        # Scroll down on HU display 10 times using the specified coordinates
        for i in range(10):
            # Click on the last name with verification from HU display
            found = click_on_device_regex(HU, click_target)
        ...
        assert found == True, f"Last name '{last_name}' from contact '{contact_with_photo}' has not been found on HU display.\n"
        save_to_notepad(f"Last name '{last_name}' from contact '{contact_with_photo}' has been found and pressed on HU display.\n")

        stdout = phone.check_contacts_photo_data_command(contact_id)
        assert rc == 0, f"Photo query command failed: {rc}\n"
        
        # Check if data15=BLOB word can be found in the command response with regex
        if re.search(r"data15=BLOB", stdout):
            success_message = f"{mobile_name} Contact Picture is displayed successfully on HU."
            ...
            assert False, f"{mobile_name} Contact Picture is not displayed on HU.\n"
