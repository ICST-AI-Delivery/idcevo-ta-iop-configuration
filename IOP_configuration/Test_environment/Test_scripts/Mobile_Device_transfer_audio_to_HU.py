"""
AI-Generated Test Script for: Mobile_Device_transfer_audio_to_HU
Generated on: 2026-03-06 10:32:20
...
2 Accept call on HU..
3 On Mobile Device 1 (connected device)  navigate to change speaker and transfer audio to Mobile Device speaker.
4 On Mobile Device 1 (connected device)  navigate to change speaker and transfer audio to HU.
- Expected Result: 3 Audio is only audible on Mobile Device 1.
4 Audio is only audible on HU.
"""
...
test_name = "Mobile_Device_transfer_audio_to_HU"
...
def main():
    ...
    try:
       ...
       # Run the command to get Bluetooth Name for HU
        ...
        # Check if audio is only audible on HU
        if found:
            success_message = f"Audio is only audible on HU - audio transfer to HU works correctly."
            ...
            assert False, f"Audio routing is not working properly - audio is not routed to HU.\n"
