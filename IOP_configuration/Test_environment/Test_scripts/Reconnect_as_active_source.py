...
test_name = "Reconnect_as_active_source"
...
def main():
    ...
    try:
        ...
        save_to_notepad(f"Enabled airplane mode on Mobile Device\n")
        time.sleep(5)
        ...
        save_to_notepad(f"Executed linkloss command to re-establish bluetooth connection\n")
        time.sleep(10)  # Wait for bluetooth connection to be re-established

        # Check test result
        found = find_word_on_device(HU, "Connected")
        if found == True:
            success_message = f"Bluetooth connection is re-established, audio is playing - test Passed.\n"
            ...
            assert False, f"Bluetooth connection was not re-established after removing from shielding or audio is not playing - test Failed\n"
        ...
        # Stop screen recording on success
