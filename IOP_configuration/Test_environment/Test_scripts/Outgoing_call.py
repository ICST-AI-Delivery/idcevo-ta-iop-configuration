"""
AI-Generated Test Script for: Outgoing_call
Generated on: 2026-03-04 10:44:21

Test Case Information:
- Precondition: Mobile Device and HU are connected.
Synchronisation is finished.
- Description: 1 Initiate an outgoing call from Mobile Device 1(paired device) to Mobile Device 3.
- Expected Result: 1 HU and Mobile Device 1 indicate outgoing call. Correct number or contact is displayed on HU.
"""
...
def main():
    ...
    try:
        ...
        # Create recordings folder and start screen recording
        ...
        time.sleep(3)  # Wait for call to be initiated
        ...
        if found:
            save_to_notepad(f"HU indicates outgoing call with correct number displayed (last 3 digits: {last_3_digits})\n")
        else:
            assert False, f"HU does not indicate outgoing call properly - last 3 digits {last_3_digits} not found\n"

        if found == True:
            success_message = f"HU and {mobile_name} indicate outgoing call correctly with proper number display"
            ...
            assert False, f"HU and {mobile_name} does not indicate outgoing call correctly with proper number display\n"
        ...
        save_to_notepad(f"Call ended on Mobile Device\n")
        time.sleep(2)

        # Run adb command on HU to go home
